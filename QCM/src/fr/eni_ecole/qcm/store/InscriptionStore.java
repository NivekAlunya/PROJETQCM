...
public class InscriptionStore {
	...
	public static void modifier (Inscription inscription){
		...
		try{
			cnx=PoolConnexion.getConnection();
			CallableStatement cstmt = cnx.prepareCall("{CALL dbo.UpdateInscription(?,?,?,?,?,?,?,?,?)}");
	...
	public static void supprimer (Inscription inscription){
		...
		try{
			cnx=PoolConnexion.getConnection();
			CallableStatement cstmt = cnx.prepareCall("{CALL dbo.deleteInscription(?)}");
			cstmt.setInt(1, inscription.getIdInscription());
			cstmt.execute();
		} catch (Exception e){
			e.printStackTrace();
	...
	public static Inscription rechercher(int idInscription) throws Exception{
		Connection cnx=null;
		PreparedStatement rqt=null;
		ResultSet rs=null;
